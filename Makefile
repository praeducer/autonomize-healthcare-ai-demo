...
dev:
	uv run uvicorn prior_auth_demo.healthcare_api_server:app --reload --port 8000
...
test-e2e:
	uv run pytest tests/ -m e2e -v --timeout=300
...
format:
	uv run ruff format src/prior_auth_demo/ tests/
	uv run ruff check --fix src/prior_auth_demo/ tests/
...
load-fhir-data:
	uv run python -m prior_auth_demo.mock_healthcare_services.load_fhir_data
...
deck: diagrams
	uv run python scripts/generate_deck.py
